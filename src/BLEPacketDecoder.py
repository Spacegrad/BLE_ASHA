...
Модуль можно импортировать и вызывать BLEPacketDecoder.parse_packet(...) или
BLEPacketDecoder.decode_and_print(...). Оставлен также интерактивный режим.
"""
...
class BLEPacketDecoder:
    ...
    def parse_packet(self, hex_string: str) -> Dict:
        """Парсит hex строку с BLE пакетом и возвращает структуру (не печатает)."""
        ...
        while position < len(self.packet_data):
            try:
                ...
                if section:
                    ...
                    # Если не можем распарсить, проверим оставшиеся байты
    ...
    def decode_bytes(self, data: bytes) -> Dict:
        """Парсит пакет, уже представленный как bytes/bytearray."""
        hex_string = data.hex().upper()
        return self.parse_packet(hex_string)

    def decode_and_print(self, hex_string: str) -> Dict:
        """Парсит и печатает результат. Возвращает тот же словарь, что parse_packet."""
...
def main():
    ...
    while True:
        ...
        if user_input.lower() in ['q', 'quit', 'exit']:
            break
        if user_input:
            decoder.decode_and_print(user_input)
